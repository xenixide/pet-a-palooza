...
<?php 
    $currYear = date('Y');
    $birthYear = date('Y', strtotime($this->session->userdata('birth')));
                                            ...
                                            <?php if($age >= 18){ ?>
                                                ...
                                                <div class="col-sm-6">
                                                    <?php
                                                        ...
                                                        $array = array('user_id'=> $id, 'pet_id'=> $item->petid);
                                                        ...
                                                        if($count == 0){
                                                    ?>
                                                        <a data-petid="<?=$item->petid?>" data-petname="<?=$item->petname?>" href="<?=base_url('user/Adopt')?>" class="btn btn-primary btn-sm btn-block adopt">Adopt</a>
                                                    <?php }else{ ?>
                                                        <a data-petid="<?=$item->petid?>" data-petname="<?=$item->petname?>" href="<?=base_url().'user/adoptCancel'?>" class="btn btn-danger btn-sm btn-block adoptCancel">Cancel Adopt</a>
                                                    <?php } ?>
                                                </div>
                                            <?php } else { ?>
                                                <a href="<?=base_url().'user/AnimalView/'?>" data-id="<?=$item->petid?>" type="button" class="btn btn-warning btn-sm btn-block viewanimal" >Know More</a>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        <button id="" type="button" class="btn btn-warning" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                $.each(data.anim, function(key, val){
                    ...
                    $html += '</div>';
                    $html += '<div class="col-sm-6">';
                        if(myArr.indexOf(val.petid) == -1){
                            $html += '<a href="<?=base_url('user/Adopt')?>" data-petid="'+val.petid+'" data-petname="'+val.petname+'" class="btn btn-primary btn-sm btn-block adopt" >Adopt</a>';
                        }else{
                            $html += '<a href="<?=base_url('user/adoptCancel')?>" data-petid="'+val.petid+'" data-petname="'+val.petname+'" class="btn btn-danger btn-sm btn-block adoptCancel">Cancel Adopt</a>';
                        }
                    }else{
                        $html += '<a href="<?=base_url().'user/AnimalView/'?>" data-id="'+val.petid+'" type="button" class="btn btn-warning btn-sm btn-block viewanimal" >Know More</a>';
                    }
...
<?php if($appform != null){ if($appform->priority < 80){ ?>
<script>
$(document).ready(function(){
    $('.adopt').click(function(e){
        e.preventDefault();
        toastr["info"]("Unfortunately, your profile doesn't meet the requirements to adopt. Thank you", "Oops!");
    });
});
</script>
<?php }else{ ?>
...
$(document).ready(function(){
    $(document).on('click', '.adopt', function(e){
        ...
        $(this).text('Cancel Adopt');
        $(this).addClass('btn-danger');
        $(this).addClass('adoptCancel');
        $(this).removeClass('btn-primary');
        $(this).removeClass('adopt');
        $(this).attr('href', '<?=base_url().'user/adoptCancel'?>')
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(res){
                toastr["success"]("Adoption Sent", "Yey!")
            }
        });
    });
    
    $(document).on('click', '.adoptCancel', function(e){
        e.preventDefault();
        $url = $(this).attr('href');
        $petid = $(this).data('petid');
        $petname = $(this).data('petname');
        ...
        $(this).removeClass('adoptCancel');
        $(this).attr('href', '<?=base_url().'user/Adopt'?>')
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(res){
                toastr["warning"]("Adoption Cancelled", ":(")
            }
        });
    });
});
...
$(document).ready(function(){
    $(document).on('click', '.adopt', function(e){
        e.preventDefault();
        toastr["info"]("Please update your adoption answers first.", "Oops!");
        $('.toast-message').append('<a href="<?=base_url('user/index')?>"><h6>Click here to Update your profile now</h6></a>')
    });
});
</script>
